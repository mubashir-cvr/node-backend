...
exports.createStockItem = (req, res, next) => {
  ...
  hasPermission(req.userId, ["createStockItem"])
    .then((hasPermission) => {
      ...
      const { item, item_type, gsm, dimention_length, dimention_breadth, unit_of_measurement, as_on_date } = req.body;
...
exports.updateStockItem = (req, res, next) => {
  ...
  const { item, item_type, gsm, dimention_length, dimention_breadth, unit_of_measurement } = req.body;
